...
//            if (member == null) throw new NoSuchElementException();
            ...
            int itemFabric = fabricToNum.getOrDefault(itemDto.getFabric(),-1);
            int itemPrint = printToNum.getOrDefault(itemDto.getPrint(),-1);
            int itemCategory = categoryToNum.getOrDefault(itemDto.getCategory(),-1);
            if (itemFabric == -1 || itemPrint == -1 || itemCategory == -1) throw new AttributeNotFoundException();
